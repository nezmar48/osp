...
extern "C" void init_kernel_paging() {
    ...
    kernel_page_directory[0] = ((unsigned long)first_page_table) | READ_WRITE | PRESENT;

    // identity map first 4 Mb of memory
    for(unsigned int i = 0; i < 1024; i++) {
        get_page(kernel_page_directory, i * 0x1000, READ_WRITE | PRESENT); //present read, write
        if (first_page_table[i] != ((i * 0x1000) | 3))
            asm ("cli; hlt; mov %0, %%eax" : : "r" (i));
    }

    init_page_table(second_page_table, READ_WRITE);
    kernel_page_directory[1] = ((unsigned long)second_page_table) | READ_WRITE | PRESENT;
